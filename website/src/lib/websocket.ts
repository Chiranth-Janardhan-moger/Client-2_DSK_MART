...
// Simple WebSocket connection for Track page
...
export const connectWebSocket = (): WebSocket | null => {
  ...
  try {
    ...
    trackingWs.onopen = () => {
      ...
      // Tell backend we're tracking - this registers us to receive location updates
      trackingWs?.send(JSON.stringify({ type: 'START_TRACKING' }));
    };
...
export const disconnectWebSocket = () => {
  if (trackingWs) {
    // Tell backend we stopped tracking
    if (trackingWs.readyState === WebSocket.OPEN) {
      trackingWs.send(JSON.stringify({ type: 'STOP_TRACKING' }));
    }
    trackingWs.close();
    trackingWs = null;
  }
